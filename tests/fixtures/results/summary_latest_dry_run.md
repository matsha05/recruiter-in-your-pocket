# PromptOps Fixture Validation Report

**Run ID:** eval_1785259017973
**Timestamp:** 2026-07-28T17:16:57.976Z
**Tier:** golden
**Prompt Version:** v1
**Contract Version:** v2
**Execution Mode:** Dry run (no model calls)

> This report validates fixture files and configuration only. It is not evidence of model output quality.

## Summary

| Metric | Value |
|:-------|:------|
| Total Fixtures | 23 |
| Validated | 23 |
| ⚠️ Warned | 0 |
| ❌ Failed | 0 |
| Fixture Validation Rate | 100.0% |

## Cost

| Metric | Value |
|:-------|:------|
| API Calls | 0 |
| Token-calculated API Cost | $0.000000 |
| Input Tokens | 0 |
| Cached Input Tokens | 0 |
| Output Tokens | 0 |
| Reasoning Tokens | 0 |
| Budget | $5.00 |

## Validated Fixtures

23 fixture files were found. Their model outputs were not evaluated.

<details>
<summary>Expand to see fixture IDs</summary>

- golden_talent_leader_high_bar_matt_shaw_current_1
- golden_finance_controller_strong_foundation_1
- golden_sales_strong_foundation_mark_sherrod_1
- anchor_elite_ml_staff_1
- anchor_elite_pm_ai_1
- anchor_elite_vp_talent_1
- recruiter_palantir_strong_foundation_1
- devops_senior_high_bar_1
- law_inhouse_thin_signal_1
- finance_not_ready_sam_helmuth_1
- synth_data_scientist_senior_elite_1
- synth_marketing_manager_vp_elite_1
- synth_software_engineer_mid_strong_1
- synth_ux_designer_director_strong_1
- synth_sales_executive_mid_foundation_1
- synth_hr_manager_director_foundation_1
- synth_finance_analyst_senior_weak_1
- synth_operations_manager_entry_weak_1
- synth_project_manager_vp_strong_1
- synth_product_manager_entry_elite_1
- career_path_elementary_educator_1
- career_path_ministry_leader_1
- frontline_retail_entry_1
</details>
