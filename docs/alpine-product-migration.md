...
## September 13: shared motion and interaction polish

The active web product now uses the canonical 140/180ms interaction timing, steady action labels, truthful upload/copy/save feedback, live reduced-motion preferences, and consistent dialog/disclosure behavior. Report navigation tracks the reading position while report prose remains still. The protected System Lab includes interactive reference states.

The audit fixed premature upload acceptance, stale parsing responses, clipboard focus/timer failures, a mobile sheet that could remain mounted after reduced motion interrupted its exit, and post-purchase report navigation that measured before its confirmation notice had a settled height. Delayed receipt saving now retains one notice and carries dismissal to the actual saved report ID. Unsaved dismissals stay local to their notice. The instrument's approved physical behavior is retained; its material guard also recognizes the optimized v2 plate with the same calibrated geometry.

Verification on the isolated optimized build at `http://127.0.0.1:3100`: **150/150 browser contracts passed**, including accessibility, auth, upload, mobile navigation, report reading, purchase/restore, delayed saving, jobs, and instrument fallback states. **22 Chromium/WebKit route and state checks passed** at 1440 and 390px, including the System Lab with a temporary local-only flag. The normal production-protected reference route returned 404 after that flag was removed. Native disclosures also passed in WebKit without intrinsic-size animation support. Design-system guardrails, ESLint, TypeScript, and the production build passed.

Screenshots, source hashes, test evidence, and the full audit are under `/tmp/riyp-motion-polish-20260913/`. Verification used synthetic data and mocked service responses; no real payment, email delivery, or private resume generation was performed. The isolated server's unmocked free-status endpoint fails closed without Redis, while the public live endpoint returned `ok: true` when checked. This audit does not revalidate PDF/email rendering or establish a new hosted release. No deployment was performed for this pass.
