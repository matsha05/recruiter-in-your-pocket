...
CREATE TABLE IF NOT EXISTS private.generation_operations (
  ...
  CONSTRAINT generation_operation_status_check CHECK (status IN ('executing', 'committed', 'terminal')),
  CONSTRAINT generation_operation_reservation_required CHECK (reservation_id IS NOT NULL)
);
...
BEGIN
  ...
  IF FOUND THEN
    IF v_operation.report_kind <> p_report_kind
      OR v_operation.request_digest <> p_request_digest
    THEN
      RAISE EXCEPTION 'generation operation payload mismatch';
    END IF;
  ...
  v_access := public.reserve_generation_access(p_user_id, v_reservation_id, p_report_kind);
