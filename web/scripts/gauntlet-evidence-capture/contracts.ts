...
export const AGREED_RUN_RECEIPT: LockedRunReceipt = Object.freeze({
  fullRunSha256: "a5843304acc6752382d4566da2dabdb0e4a9254aeccb434c7d327f58e0a3cefe",
  runId: "eval_1785271781375",
  generatedAt: "2026-07-28T20:51:41.424Z",
  executionMode: "live",
  model: "gpt-5.6-luna",
  reasoningEffort: "low",
  promptVersion: "resume_v2_luna_low_human_voice_1",
...
function assertExact(value: unknown, expected: unknown, label: string) {
  if (value !== expected) throw new Error(`${label} does not match the locked run receipt`);
}
...
export function sanitizeHistoricalRun(
  bytes: Buffer,
  manifest: GauntletManifest,
  receipt: LockedRunReceipt = AGREED_RUN_RECEIPT,
  approvedCases: readonly ApprovedCaseFixture[] = APPROVED_CASE_FIXTURES,
): SanitizedHistoricalSource {
  assertExact(sha256(bytes), receipt.fullRunSha256, "full-run SHA-256");
  ...
  assertSafeManifest(manifest, receipt.selectedResults, approvedCases);

  const metadata = parsed.metadata;
  assertExact(metadata.run_id, receipt.runId, "run id");
  assertExact(metadata.timestamp, receipt.generatedAt, "run timestamp");
  assertExact(metadata.execution_mode, receipt.executionMode, "execution mode");
  assertExact(metadata.model, receipt.model, "model");
  assertExact(metadata.reasoning_effort, receipt.reasoningEffort, "reasoning effort");
  assertExact(metadata.prompt_version_hash, receipt.promptVersion, "prompt version");
  assertExact(metadata.resume_prompt_sha256, receipt.canonicalPromptSha256, "resume prompt SHA-256");
  assertExact(metadata.contract_version, receipt.contractVersion, "contract version");
  assertExact(parsed.summary.total, receipt.totalResults, "summary total");
  assertExact(parsed.results.length, receipt.totalResults, "result count");
  ...
  const results = manifest.cases.map((testCase) => {
    ...
    if (selected.response_model !== receipt.model) {
  ...
  if (new Set(results.map((result) => result.fixtureId)).size !== receipt.selectedResults) {
...
export async function sanitizeHistoricalRunFiles(input: {
  ...
  const [sourceStats, manifestStats] = await Promise.all([lstat(input.sourcePath), lstat(input.manifestPath)]);
  ...
  const sanitized = sanitizeHistoricalRun(
    bytes,
    manifest,
    input.receipt ?? AGREED_RUN_RECEIPT,
    input.approvedCases ?? APPROVED_CASE_FIXTURES,
  );
  const serialized = serialize(sanitized);
  if (input.write) {
    if (!input.outputPath) throw new Error("--output is required with --write");
