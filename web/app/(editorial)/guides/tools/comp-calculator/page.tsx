...
function BigNumberInput({ label, value, onChange, prefix = "$", suffix, hint }: {
    ...
    return (
        <div className="space-y-1.5">
            <label className="text-xs font-semibold uppercase riyp-track-010 text-muted-foreground">{label}</label>
                        ...
                        <div className="grid grid-cols-4 gap-2 p-3 bg-muted/30 rounded-lg">
                            {[1, 2, 3, 4].map((year, i) => (
                                <div key={year} className="space-y-1">
                                    <label className="text-xs font-semibold uppercase riyp-track-010 text-muted-foreground">Y{year} %</label>
                                    <input
                                        type="number"
                                        value={schedule[i] || 0}
...
function OfferInputCard({ offer, onChange, onRemove, colorIndex, canRemove }: {
    ...
    return (
        <motion.div layout initial={false} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }}
            ...
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                    ...
                    {offer.isCurrentJob && <span className="text-xs font-semibold uppercase riyp-track-010 text-muted-foreground bg-muted px-2 py-0.5 rounded">Baseline</span>}
                </div>
            ...
            <div className="mt-4 pt-4 border-t border-border/20">
                <div className="flex items-baseline justify-between">
                    <span className="text-xs font-semibold uppercase riyp-track-010 text-muted-foreground">4-Year Total</span>
...
function StackedBarChart({ offers }: { offers: OfferData[] }) {
    ...
    return (
        <div className="space-y-4">
            <h3 className="text-xs font-semibold uppercase riyp-track-010 text-muted-foreground">Year-by-Year Breakdown</h3>
...
function DataTable({ offers }: { offers: OfferData[] }) {
    ...
    return (
        <div className="space-y-3">
            <h3 className="text-xs font-semibold uppercase riyp-track-010 text-muted-foreground">Detailed Breakdown</h3>
...
function SummaryTable({ offers }: { offers: OfferData[] }) {
    ...
    return (
        <div className="space-y-3">
            <h3 className="text-xs font-semibold uppercase riyp-track-010 text-muted-foreground">4-Year Ranking</h3>
            <div className="space-y-2">
                {sorted.map((offer, i) => {
                        ...
                        <div key={offer.id} className="relative">
                            ...
                            <div className="relative flex items-center justify-between p-3 rounded-lg">
                                <div className="flex items-center gap-3">
                                    ...
                                    {offer.isCurrentJob && <span className="text-xs font-semibold uppercase riyp-track-010 text-muted-foreground bg-muted px-2 py-0.5 rounded">Current</span>}
                                    {isWinner && validOffers.length > 1 && <span className="text-xs font-semibold uppercase riyp-track-010 text-white bg-brand px-2 py-0.5 rounded">Best</span>}
                                </div>
    ...
    return (
        <>
            ...
            <div className="mx-auto max-w-6xl px-6 py-12 md:py-20">
                <header className="mb-12 grid gap-8 border-b border-border/70 pb-12 lg:grid-cols-3 lg:items-end">
                    <div className="lg:col-span-2">
                        <div className="mb-5 text-xs font-semibold uppercase riyp-track-010 text-brand">Offer calculator</div>
                        <h1 className="max-w-4xl font-display text-5xl font-medium text-foreground riyp-leading-094 riyp-stretch-92 riyp-track-n055 md:text-7xl">
                            The biggest offer is not always the best one.
                        </h1>
                    </div>
                    <div>
                        <p className="text-lg leading-8 text-muted-foreground">
                            Put salary, bonus, equity, vesting, and one-time money on the same four-year timeline. Then compare what you would actually receive.
                        </p>
                        <p className="mt-4 text-sm leading-6 text-muted-foreground">
                            No account. Nothing saved. Change any assumption and the comparison updates here.
                        </p>
                    </div>
                </header>
                ...
                <div className="grid lg:grid-cols-[1fr,420px] gap-6">
                    ...
                    <div className="lg:sticky lg:top-24 lg:self-start space-y-6">
                        {hasData ? (
                            ...
                            <div className="border border-border/70 bg-background p-8 text-center">
