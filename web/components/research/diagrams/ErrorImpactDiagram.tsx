...
export function ErrorImpactDiagram({ figureNumber = 1 }: { figureNumber?: number }) {
    return (
        <DiagramFigure className="max-w-[43rem]" label="Inference chain from a resume error to a possible callback penalty">
            <DiagramFrame>
                <EvidenceHeader index={String(figureNumber).padStart(2, "0")} label="From typo to judgment" title="A small error can change the reader's impression." note="The mistake may be minor. The reader may still make broader assumptions about care or writing ability." />
                <SequenceTrace steps={[
            ...
            <DiagramCaption kicker={`Fig. ${figureNumber} / After the typo`} title="The larger effect comes from what the reader assumes the mistake means." />
        </DiagramFigure>
    );
}
