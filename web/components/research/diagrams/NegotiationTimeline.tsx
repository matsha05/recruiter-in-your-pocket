...
export function NegotiationTimeline() {
    ...
    return (
        <figure className="w-full my-10 overflow-x-auto">
            <div className="min-w-[800px] relative">
                <div className="absolute top-[12px] left-[8%] right-[8%] h-px bg-border/60" />

                <div className="grid grid-cols-5 gap-4">
                    {steps.map((step) => (
                        <div key={step.title} className="flex flex-col items-center text-center">
                            <div className={`w-3 h-3 border border-border/60 ${step.highlight ? "bg-brand/40" : "bg-foreground/20"}`} />
