...
import styles from "./FixCanvas.module.css";
...
export function FixCanvas({
  ...
  if (dismissed) {
    return (
      <article className="riyp-border-paper-line border-t py-7 first:border-t-0">
        <div className="flex flex-col gap-4 rounded-xl bg-proof px-5 py-5 sm:flex-row sm:items-center sm:justify-between">
  ...
  return (
    <article id={`section-fix-${index + 1}`} className={styles.root}>
      <div className={styles.layout}>
        <div className="flex items-baseline gap-3 sm:block">
          <p className="text-xs font-semibold uppercase riyp-track-015 text-brand">Fix {String(index + 1).padStart(2, "0")}</p>
        ...
        <div className="min-w-0">
          ...
          {hasSuggestedLine && <div className="mt-5 hidden rounded-xl bg-proof/50 px-4 py-4 sm:block sm:px-5">
            <LiftedTrace items={fixTrace} progress={traceProgress} ariaLabel={`How fix ${index + 1} moves from resume evidence to clearer wording`} className={styles.trace} compact />
          </div>}
          ...
          {draftSource && (
            <div className={styles.source}>
              ...
              <p className={styles.sourceText}>“{visibleSource}”</p>
          ...
          <div className={`${styles.reasoning} ${hasSuggestedLine ? styles.reasoningColumns : ""}`}>
            <div className={styles.reasoningItem}>
              <p className={styles.label}>Why it matters</p>
              <p className={styles.body}>{fix.why || "This example needs more detail about what you did and what changed."}</p>
            </div>
            {hasSuggestedLine && <div className={styles.reasoningItem}>
              <p className={styles.label}>{placeholderKeys.length > 0 ? "Details to add" : "Before you use this"}</p>
              {placeholderKeys.length > 0 && rewrite?.enhancement_note && (
                <p className="mt-3 text-base leading-6 text-foreground">{rewrite.enhancement_note}</p>
              )}
              {isSample ? (
                placeholderKeys.length > 0 ? (
                  !rewrite?.enhancement_note && <ul className="text-base mt-3 grid list-disc gap-2 pl-5 leading-6 text-foreground/80">
                    {placeholderKeys.map((key) => <li key={key}>{questionForPlaceholder(key)}</li>)}
                  </ul>
                ) : (
                  <p className="text-base mt-3 leading-6 text-foreground">This example uses only the details in the original line.</p>
                )
              ) : placeholderKeys.length > 0 ? (
                <div className="mt-4 grid gap-3">
                  {placeholderKeys.map((key) => (
                    <label key={key} className="grid gap-1.5 text-sm font-semibold text-foreground">
                ...
                <p className="text-base mt-3 leading-6 text-foreground">Check that the suggestion still describes the work you did.</p>
              )}
            </div>}
          </div>
          ...
          {hasSuggestedLine && awaitingFacts ? (
            ...
            <div className={styles.wording}>
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-xs font-semibold uppercase riyp-track-015 text-brand">{isSample ? "Example wording" : editingOriginal ? "Your wording" : "Suggested wording"}</p>
              ...
              {editing && !isSample ? (
                <textarea value={draft} onChange={(event) => setDraft(event.target.value)} className={styles.draftInput} aria-label={`Edit suggested line ${index + 1}`} />
              ) : (
