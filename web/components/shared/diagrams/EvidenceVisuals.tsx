...
const toneClasses: Record<NonNullable<ProcessStep["tone"]>, string> = {
    ...
    caution: "border-foreground/60 text-foreground",
};
                ...
                {steps.map((step, index) => {
                    ...
                    return (
                        <li key={`${step.label}-${step.title}`} className="relative border-l border-line py-5 pl-6 first:pt-0 last:pb-0 md:border-l-0 md:border-t md:px-4 md:pb-0 md:pt-7 md:first:pl-0 md:last:pr-0">
                            ...
                            <div className="mt-2 font-display text-xl font-medium tracking-tight text-foreground">{step.title}</div>
                            <p className="mt-2 text-data text-muted-foreground">{step.detail}</p>
...
const comparisonTone: Record<NonNullable<ComparisonSide["tone"]>, string> = {
    ...
    complete: "bg-surface-proof text-foreground",
};
                ...
                {[left, right].map((side, index) => {
                    ...
                    return (
                        <section key={side.eyebrow} className={cn("min-h-full px-5 py-7 md:px-7 md:py-9", index === 0 && "border-b border-line md:border-b-0 md:border-r", comparisonTone[tone])}>
                            ...
                            <h4 className="mt-3 font-display text-report-title tracking-tight text-foreground">{side.title}</h4>
                            <ul className="mt-6 divide-y divide-line">
                                {side.items.map((item) => (
                                    <li key={item} className="flex gap-3 py-3 text-data text-muted-foreground">
                                        <span className={cn("mt-2.5 h-0.5 w-4 shrink-0", tone === "quiet" ? "bg-muted-foreground/50" : "bg-brand")} aria-hidden="true" />
                                        {item}
                                    </li>
                                ))}
                            </ul>
                ...
                return (
                    <li key={`${step.label}-${step.title}`} className="grid grid-cols-[2.5rem_1fr] gap-4 md:grid-cols-[3rem_0.72fr_1.28fr] md:gap-6">
                        <div className="relative flex justify-center">
                            ...
                            <span className={cn("relative z-10 flex size-8 items-center justify-center rounded-lg border bg-paper text-label tabular-nums", toneClasses[tone])}>{String(index + 1).padStart(2, "0")}</span>
                        </div>
                        <div className="pb-7 md:pb-8">
                            <div className={cn("riyp-evidence-label", toneClasses[tone].split(" ").at(-1))}>{step.label}</div>
                            <div className="mt-1 font-display text-xl font-medium tracking-tight text-foreground">{step.title}</div>
                        </div>
                        {step.detail ? <p className="col-start-2 -mt-5 pb-7 text-data text-muted-foreground md:col-start-3 md:mt-0 md:pb-8">{step.detail}</p> : null}
                    </li>
                );
            })}
        </ol>
...
export function FrameworkStrip({ steps, connector = "+", example }: { steps: FrameworkStep[]; connector?: string; example?: ReactNode }) {
    return (
        <div className="px-5 py-7 md:px-7 md:py-9">
            <div className="grid gap-4 sm:grid-cols-[repeat(var(--framework-count),minmax(0,1fr))]" style={{ "--framework-count": steps.length } as React.CSSProperties}>
                {steps.map((step, index) => (
                    <div key={step.symbol} className="relative border-t border-line pt-4">
                        <div className={cn("font-display text-section-title", step.focus ? "text-brand" : "text-foreground")}>{step.symbol}</div>
            ...
            {example ? <div className="mt-8 rounded-xl border border-line bg-proof px-5 py-4 text-data text-muted-foreground">{example}</div> : null}
        </div>
    );
}
            ...
            <table className="riyp-evidence-table w-full border-collapse text-left">
                <thead>
                    <tr className="border-b border-line">
                        {columns.map((column, index) => <th key={column} className={cn("riyp-evidence-label pb-3 pr-5", index === 0 ? "text-muted-foreground" : "text-foreground")}>{column}</th>)}
                    </tr>
                </thead>
                <tbody className="divide-y divide-line">
                    {rows.map((row) => (
                        <tr key={row.label}>
                            <th scope="row" className="py-4 pr-5 text-data font-medium text-foreground">{row.label}</th>
                            {row.values.map((value, index) => <td key={`${row.label}-${index}`} data-label={columns[index + 1]} className={cn("py-4 pr-5 text-data tabular-nums", row.emphasis === index ? "font-semibold text-brand" : "text-muted-foreground")}>{value}</td>)}
                        </tr>
                    ))}
                </tbody>
            </table>
