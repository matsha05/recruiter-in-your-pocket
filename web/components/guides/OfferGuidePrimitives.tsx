...
export function GuideEyebrow({ children }: { children: ReactNode }) {
    return <div className="text-xs font-semibold uppercase riyp-track-010 text-brand">{children}</div>;
}
    ...
    return (
        <section id={id} className="scroll-mt-28 border-t border-border py-12 sm:py-16">
            <div className="grid gap-8 lg:grid-cols-4 lg:gap-12">
                <div className="font-display text-5xl riyp-weight-520 text-brand/35">{number}</div>
    ...
    return (
        <details open={defaultOpen || undefined} className="group border-t border-border last:border-b">
            <summary className="flex min-h-16 cursor-pointer list-none items-center justify-between gap-5 py-4 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-4 [&::-webkit-details-marker]:hidden">
                <div>
                    {eyebrow ? <div className="mb-1 text-xs font-semibold uppercase riyp-track-010 text-brand">{eyebrow}</div> : null}
                    <span className="font-display text-xl riyp-weight-560 leading-tight text-foreground">{title}</span>
                </div>
                <span className="flex size-10 shrink-0 items-center justify-center rounded-full border border-border bg-background transition group-open:rotate-180">
...
function HandoffNode({ label, text }: { label: string; text: string }) {
    return (
        <div className="border-l-2 border-brand pl-4">
            <div className="text-xs font-semibold uppercase riyp-track-010 text-brand">{label}</div>
...
export function WhatItGives({ children }: { children: ReactNode }) {
    return (
        <div className="mt-4 flex gap-3 text-sm leading-6 text-muted-foreground">
            ...
            <p><strong className="font-semibold text-foreground">What this gives them:</strong> {children}</p>
        </div>
    );
}
    ...
    return (
        <div className="border border-border bg-foreground p-5 text-background sm:p-8">
            <div className="mb-6 flex items-center justify-between gap-4">
                <div>
                    <div className="text-xs font-semibold uppercase riyp-track-010 text-brand">The honest offer</div>
                    <div className="mt-2 font-display text-2xl riyp-weight-520">Four numbers, not one</div>
                </div>
                <Info aria-hidden className="size-6 text-brand" weight="fill" />
            </div>
