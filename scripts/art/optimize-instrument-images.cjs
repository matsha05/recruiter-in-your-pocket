/* Versioned delivery assets. Original approved artwork stays untouched. */
...
async function main() {
    ...
    // Same calibrated dimensions, with visually reviewed photographic compression.
    const compact = path.join(instrument, 'clean-plate.v2.webp');
    await sharp(plate).webp({ quality: 92, effort: 6 }).toFile(compact);
    if ((await sharp(compact).metadata()).width !== 1536) throw new Error('Plate calibration width changed');
    ...
    for (const file of [delivered, compact, logos]) console.log(path.basename(file), (await fs.stat(file)).size);
}
